...
-- Step 4b: Canonicalize organization scoping on photo metadata.
-- This migration applies org-based RLS to `photo_metadata`, so ensure the
-- table has a first-class `organization_id` column before policy creation.
