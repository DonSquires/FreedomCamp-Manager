-- ============================================================================
-- Restore Complete Observation Pipeline in safe_insert_observation
-- Date: 2026-04-04 (sequence 2)
--
-- Problem:
--   The safe_insert_observation RPC (created by 20260404000001) uses
--   SET LOCAL session_replication_role = 'replica' which bypasses ALL 7
--   triggers on the observations table.  This means observations inserted
--   via the fallback path have:
--     - No vehicle details populated (trigger_populate_observation_from_canonical)
--     - Hardcoded is_compliant=true regardless of actual compliance
--     - No compliance_results row (trigger_auto_create_compliance_result)
--     - No breach_alerts created
--     - No canonical_vehicles stats updated
--     - No vehicle_monthly_stays tracking
--
-- Fix:
--   Replace safe_insert_observation with a version that:
--     1. Inserts the observation with triggers disabled (to avoid COALESCE crash)
--     2. Manually runs the equivalent of each trigger function inline
--     3. Returns the complete observation with compliance data populated
--
-- This restores the Feb 2025 working trigger chain logic
-- (see migrations 20250203000002 through 20250214000001) but executes it
-- inline instead of via triggers, avoiding the COALESCE type-mismatch issue.
--
...
-- ============================================================================
-- Replace safe_insert_observation with pipeline-complete version
-- ============================================================================
...
BEGIN
  ...
  -- (Equivalent of trigger_auto_create_compliance_result from 20250214000001)
  -- ========================================================================
  BEGIN
  ...
  -- (Equivalent of trigger_update_canonical_stats_v2)
  -- ========================================================================
  BEGIN
  ...
  -- (Equivalent of trigger on compliance_results that creates breach_alerts)
  -- ========================================================================
...
COMMENT ON FUNCTION public.safe_insert_observation(jsonb) IS
  'Pipeline-complete observation insert: bypasses broken triggers but runs '
  'equivalent compliance evaluation, compliance_results creation, canonical '
  'stats update, and breach_alert creation inline.  Based on the working '
  'Feb 2025 trigger chain (20250203000002 through 20250214000001).';
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '═══════════════════════════════════════════════════════════════';
  RAISE NOTICE '  20260404000002 — safe_insert_observation PIPELINE RESTORED';
  RAISE NOTICE '═══════════════════════════════════════════════════════════════';
  RAISE NOTICE '  The safe_insert_observation RPC now performs:';
  RAISE NOTICE '    1. Populate vehicle details from canonical_vehicles  ✓';
  RAISE NOTICE '    2. Evaluate compliance (zone matrix rules)           ✓';
  RAISE NOTICE '    3. Insert observation (triggers bypassed)            ✓';
  RAISE NOTICE '    4. Create compliance_results row                     ✓';
  RAISE NOTICE '    5. Update canonical_vehicles stats                   ✓';
  RAISE NOTICE '    6. Create breach_alert if non-compliant              ✓';
  RAISE NOTICE '';
  RAISE NOTICE '  Based on working Feb 2025 trigger chain:';
  RAISE NOTICE '    20250203000002 — populate_observation_from_canonical';
  RAISE NOTICE '    20250214000001 — auto_create_compliance_result';
  RAISE NOTICE '═══════════════════════════════════════════════════════════════';
END;
$$;
