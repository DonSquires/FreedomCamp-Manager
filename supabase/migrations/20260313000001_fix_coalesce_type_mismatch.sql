-- ============================================================================
-- Fix: COALESCE types integer and text cannot be matched
-- Date: 2026-03-13
--
-- Symptoms:
--   • Scan pipeline fails on observation INSERT with error:
--     "COALESCE types integer and text cannot be matched"
--   • Error occurs in the BEFORE INSERT trigger `auto_evaluate_compliance()`
--
-- Root cause:
--   The trigger function uses patterns like:
--     COALESCE(NEW.column::text, '')
--   When PostgreSQL evaluates this, it tries to match types between the
--   column (which may be INTEGER) and the empty string literal (TEXT) BEFORE
--   applying the ::text cast. This causes a type mismatch error.
--
-- Solution:
--   1. Ensure compliance columns have correct INTEGER/BOOLEAN types
--   2. Replace the trigger function with an ultra-defensive version that:
--      - Uses IF/THEN instead of COALESCE for NULL checks
--      - Uses pg_typeof() to detect column type at runtime
--      - Wraps all conversions in BEGIN/EXCEPTION blocks
...
-- ============================================================================
-- PART 1: Ensure compliance columns exist with correct types
-- ============================================================================
...
-- ============================================================================
-- PART 2: Fix column types if they have drifted to TEXT
-- ============================================================================
...
BEGIN
  ...
  IF v_type IS NOT NULL AND v_type <> 'integer' THEN
    ...
    $x$;
    RAISE NOTICE 'Fixed nights_stayed_this_month type: % → integer', v_type;
  END IF;
  ...
  IF v_type IS NOT NULL AND v_type <> 'integer' THEN
    ...
    $x$;
    RAISE NOTICE 'Fixed consecutive_nights type: % → integer', v_type;
  END IF;
  ...
  IF v_type IS NOT NULL AND v_type <> 'boolean' THEN
    ...
    $x$;
    RAISE NOTICE 'Fixed is_compliant type: % → boolean', v_type;
  END IF;
  ...
  IF v_type IS NOT NULL AND v_type <> 'boolean' THEN
    ...
    $x$;
    RAISE NOTICE 'Fixed self_contained type: % → boolean', v_type;
  END IF;
END;
$$;

-- Set/restore column defaults
ALTER TABLE public.observations
...
-- ============================================================================
-- PART 3: Replace trigger function with ultra-defensive version
-- ============================================================================
-- Key changes from previous versions:
--   1. Uses IF/THEN for NULL checks instead of COALESCE(column, literal)
--   2. Uses pg_typeof() to detect column type at runtime
--   3. Wraps ALL type conversions in BEGIN/EXCEPTION blocks
--   4. Assigns sanitized values to local variables first
--   5. NEVER mixes INTEGER and TEXT in COALESCE arguments

-- Drop all legacy triggers that may exist
...
BEGIN
  -- ============================================================================
  -- ULTRA-DEFENSIVE TYPE CONVERSION
  -- Handles INTEGER, TEXT, or NULL column values without COALESCE type mismatch.
  -- Each block uses IF/THEN instead of COALESCE to avoid PostgreSQL trying to
  -- match incompatible types in COALESCE arguments.
  -- ============================================================================
  ...
  BEGIN
    ...
    ELSE
      ...
      v_temp_text := regexp_replace(NEW.nights_stayed_this_month::text, '[^0-9]', '', 'g');
      IF v_temp_text = '' OR v_temp_text IS NULL THEN
        v_nights_stayed := 0;
      ELSE
        v_nights_stayed := v_temp_text::integer;
      END IF;
    END IF;
    -- Ensure non-negative
  ...
  BEGIN
    ...
    ELSE
      v_temp_text := regexp_replace(NEW.consecutive_nights::text, '[^0-9]', '', 'g');
      IF v_temp_text = '' OR v_temp_text IS NULL THEN
        v_consecutive := 0;
      ELSE
        v_consecutive := v_temp_text::integer;
      END IF;
    END IF;
  ...
  -- Note: Using COALESCE on v_matrix fields is safe because they come from
  -- our SELECT and are guaranteed to be proper types (BOOLEAN from zones table).
  -- ============================================================================
...
COMMENT ON FUNCTION public.auto_evaluate_compliance() IS
  ...
  'breach_type, breach_reason. Uses ultra-defensive type conversion with '
  'IF/THEN and pg_typeof() instead of COALESCE to prevent type mismatch errors.';
...
BEGIN
  ...
  RAISE NOTICE '';
  RAISE NOTICE '═══════════════════════════════════════════════════════════════';
  RAISE NOTICE '  20260313000001_fix_coalesce_type_mismatch - COMPLETE';
  RAISE NOTICE '═══════════════════════════════════════════════════════════════';
