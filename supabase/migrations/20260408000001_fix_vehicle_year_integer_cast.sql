-- ============================================================================
-- Fix: vehicle_year type mismatch in safe_insert_observation
-- Date: 2026-04-08
--
-- Problem:
--   safe_insert_observation fails with:
--     "INSERT failed: column "vehicle_year" is of type integer but expression
--      is of type text"
--
--   Root cause:
...
--
--   The function declares  v_vehicle_year TEXT  (correctly, to receive the
--   text value from canonical_vehicles), then inserts it directly into
--   observations.vehicle_year (INTEGER) — PostgreSQL rejects the implicit
--   text → integer conversion.
--
-- Fix:
--   In the INSERT VALUES clause, cast v_vehicle_year to INTEGER with a safe
--   regex guard so non-numeric text (e.g. NULLs, stale junk data) produces
...
DECLARE
  ...
  v_vehicle_year    TEXT;   -- TEXT because canonical_vehicles.vehicle_year is TEXT
...
BEGIN
  ...
  BEGIN
    INSERT INTO observations (
      ...
      v_vehicle_model,
      v_vehicle_color,
      -- observations.vehicle_year is INTEGER; canonical_vehicles.vehicle_year is TEXT.
      -- Use a safe regex cast: non-numeric text (or NULL) becomes NULL.
      CASE WHEN v_vehicle_year ~ '^\d+$' THEN v_vehicle_year::integer ELSE NULL END,
      v_self_contained,
      v_sc_expiry,
      v_nights_stayed,
      v_consecutive,
...
COMMENT ON FUNCTION public.safe_insert_observation(jsonb) IS
  'Pipeline-complete observation insert. Safely casts canonical_vehicles.vehicle_year '
  '(TEXT) to observations.vehicle_year (INTEGER) via regex guard. Does NOT use '
  'session_replication_role. Pre-populates compliance fields inline and lets the '
  'exception-safe trg_auto_evaluate_compliance BEFORE trigger confirm/refine them. '
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '═══════════════════════════════════════════════════════════════';
  RAISE NOTICE '  20260408000001 — safe_insert_observation vehicle_year FIX';
  RAISE NOTICE '═══════════════════════════════════════════════════════════════';
  RAISE NOTICE '  canonical_vehicles.vehicle_year is TEXT.';
  RAISE NOTICE '  observations.vehicle_year is INTEGER.';
  RAISE NOTICE '  Fix: CASE WHEN v_vehicle_year ~ ''^\\d+$'' THEN';
  RAISE NOTICE '         v_vehicle_year::integer ELSE NULL END';
  RAISE NOTICE '  Non-numeric or NULL vehicle_year values become NULL.';
  RAISE NOTICE '═══════════════════════════════════════════════════════════════';
END;
$$;
