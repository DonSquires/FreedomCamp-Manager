-- Training assignment reminders + manager audit support
-- Adds a reminder ledger and RPC so managers can nudge officers to complete
-- training while preserving a durable audit trail.

CREATE EXTENSION IF NOT EXISTS pg_net;

CREATE TABLE IF NOT EXISTS public.training_assignment_reminders (
  ...
  reminder_type TEXT NOT NULL DEFAULT 'in_app'
    CHECK (reminder_type IN ('in_app', 'email', 'sms', 'escalation')),
  message TEXT,
...
CREATE INDEX IF NOT EXISTS idx_training_assignment_reminders_org_assignment
  ON public.training_assignment_reminders(organization_id, assignment_id, sent_at DESC);

CREATE INDEX IF NOT EXISTS idx_training_assignment_reminders_officer
  ON public.training_assignment_reminders(officer_id, sent_at DESC);

CREATE INDEX IF NOT EXISTS idx_training_assignment_reminders_delivery_status
  ON public.training_assignment_reminders(organization_id, delivery_status, sent_at DESC);

ALTER TABLE public.training_assignment_reminders ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "admins manage training_assignment_reminders" ON public.training_assignment_reminders;
CREATE POLICY "admins manage training_assignment_reminders"
  ON public.training_assignment_reminders FOR ALL
  USING (
    EXISTS (
      SELECT 1
      FROM public.user_profiles up
      WHERE up.id = auth.uid()
        AND up.organization_id = training_assignment_reminders.organization_id
...
DROP POLICY IF EXISTS "officers read own training_assignment_reminders" ON public.training_assignment_reminders;
CREATE POLICY "officers read own training_assignment_reminders"
  ON public.training_assignment_reminders FOR SELECT
...
CREATE OR REPLACE FUNCTION public.send_bulk_training_assignment_reminders(
  p_organization_id UUID,
  p_status_filter TEXT[] DEFAULT ARRAY['overdue']::TEXT[],
...
BEGIN
  ...
  IF v_actor_role NOT IN ('admin', 'admin_officer', 'master', 'grand_master') THEN
    RAISE EXCEPTION 'Not allowed to send bulk assignment reminders';
  END IF;
  ...
  IF p_organization_id <> v_actor_org AND v_actor_role NOT IN ('master', 'grand_master') THEN
    RAISE EXCEPTION 'Cross-org reminder action requires master/grand_master role';
  END IF;

  v_type := lower(COALESCE(NULLIF(btrim(p_reminder_type), ''), 'in_app'));
  IF v_type NOT IN ('in_app', 'email', 'sms', 'escalation') THEN
    v_type := 'in_app';
  END IF;

  FOR r_assignment IN
    SELECT ta.id, ta.title, ta.due_at
    FROM public.training_assignments ta
    WHERE ta.organization_id = p_organization_id
      AND ta.status = ANY(COALESCE(p_status_filter, ARRAY['overdue']::TEXT[]))
      AND ta.status <> 'completed'
      AND (p_due_before IS NULL OR ta.due_at IS NULL OR ta.due_at <= p_due_before)
  LOOP
    v_targeted := v_targeted + 1;

    PERFORM public.send_training_assignment_reminder(
      p_assignment_id := r_assignment.id,
      p_reminder_type := v_type,
      p_message := CASE
        WHEN p_message_template IS NOT NULL AND btrim(p_message_template) <> '' THEN
          replace(
            replace(
              replace(p_message_template, '{{title}}', COALESCE(r_assignment.title, 'assigned training')),
              '{{due_at}}', COALESCE(to_char(r_assignment.due_at AT TIME ZONE 'Pacific/Auckland', 'DD Mon YYYY HH24:MI NZST'), 'unspecified')
            ),
            '{{assignment_id}}', r_assignment.id::text
          )
        ELSE NULL
      END,
  ...
  assignments_targeted := v_targeted;
...
REVOKE ALL ON FUNCTION public.send_bulk_training_assignment_reminders(UUID, TEXT[], TIMESTAMPTZ, TEXT, TEXT, UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.send_bulk_training_assignment_reminders(UUID, TEXT[], TIMESTAMPTZ, TEXT, TEXT, UUID) TO authenticated;
...
COMMENT ON FUNCTION public.send_bulk_training_assignment_reminders(UUID, TEXT[], TIMESTAMPTZ, TEXT, TEXT, UUID) IS
  'Creates reminders in bulk for assignments matching status and due filters.';
