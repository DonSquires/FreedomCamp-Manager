 ...
 *   4. If any zones changed, notify admin users via the `admin_notifications` table
 *      (or a Supabase Realtime broadcast on channel `doc-sync-updates`).
 *
...
/**
 * Upsert a single zone row into public.zones.
 * Uses external_id + source as the conflict target.
 */
...
async function broadcastChanges(
  ...
  await supabase.channel('doc-sync-updates').send({
    type: 'broadcast',
    event: 'zones_updated',
    payload: {
      source,
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Resolve caller identity for the log (service-role calls have no JWT sub)
