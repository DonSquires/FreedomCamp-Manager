...
1. Dispatch Job Log (B-160)
   ...
   - Role gate: admin, admin_officer, master
   - Viewer for dispatch_jobs; KPIs (Total/Completed/SLA Breached/Critical Priority); status/priority/title/date filters; assignment, SLA, zone, completion detail expand

2. Enforcement Action Log (B-161)
   - Route: /enforcement-actions-log
   - Role gate: admin, admin_officer, master
   - Viewer for enforcement_actions; KPIs (Total/Completed/Pending/Action Types); status/action_type/plate/date filters; assignment, outcome, observation, compliance result detail expand
...
1. Breach Alert Log (B-172)
   - Route: /breach-alerts-log
   - Role gate: admin, admin_officer, master
   - Viewer for breach_alerts; KPIs (Total/Open/Resolved/Notified); status/notification/plate filters; assignment and resolution detail expand

2. Fixed Camera Log (B-173)
   - Route: /fixed-cameras-log
   - Role gate: admin, admin_officer, master
   - Viewer for fixed_cameras; KPIs (Total/Online-Active/With Stream/With Snapshot); status/type/search filters; location and endpoint detail expand

3. Flagged Vehicle Log (B-174)
