...
## Crossover Kickoff (2026-04-02)

Status:

- Rebuild alignment changes committed and pushed to branch `copilot/rename-app-to-reflect-new-focus`.
- Full build validation passed (`bun run build`).
- Vercel and Expo prerequisites are present in repo configuration and documentation:
	- `vercel.json` includes SPA rewrite and security headers.
	- `mobile-app/app.json` contains valid Expo `projectId` and `updates.url`.
	- `mobile-app/eas.json` contains `development`, `preview`, and `production` profiles.

Current blocker in this execution environment:

- `vercel`, `eas`, and `supabase` CLIs are not installed in the active container.

Immediate crossover commands (run from a workstation/runner with CLIs installed):

```bash
# Web crossover (Vercel)
vercel --version
vercel pull --yes --environment=preview
vercel build
vercel deploy --prebuilt

# Mobile crossover (Expo/EAS)
cd mobile-app
eas --version
eas build --platform android --profile preview
eas build --platform ios --profile preview

# Migration crossover (Supabase)
cd /workspaces/FreedomCamp-Manager
supabase --version
supabase db push --linked --include-all --dry-run
```

Exit criteria for kickoff completion:

- Preview deployment URL generated on Vercel and smoke-tested.
- Android and iOS preview build IDs generated by EAS.
- Supabase dry-run migration output reviewed with no unexpected drift.
