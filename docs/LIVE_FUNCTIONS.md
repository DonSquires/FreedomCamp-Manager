...
| `populate_observation_from_canonical()` | observations | BEFORE | INSERT | Looks up `canonical_vehicles` by `plate_number`; denormalises `vehicle_make`, `vehicle_model`, `vehicle_color`, `vehicle_year`, `self_contained`, `is_homeless` onto `NEW` |
