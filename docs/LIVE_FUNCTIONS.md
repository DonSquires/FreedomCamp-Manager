...
| `populate_observation_from_canonical()` | observations | BEFORE | INSERT | Looks up `canonical_vehicles` by `plate_number`; denormalises `vehicle_make`, `vehicle_model`, `vehicle_color`, `vehicle_year` (both `canonical_vehicles.vehicle_year` and `observations.vehicle_year` are INTEGER since migration 20260411000003 — no TEXT→INTEGER cast required), `self_contained`, `self_contained_expiry` onto `NEW`. Top-level `EXCEPTION` block prevents type drift from blocking INSERTs. |
