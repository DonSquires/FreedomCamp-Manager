...
| Bob RunPod self-test fails with `Process from config.webServer was not able to start` | Worker clone dependencies are missing or failed to install, so Playwright cannot launch Vite webServer | Ensure worker repo dependency install succeeds before test execution and confirm `node_modules/vite/bin/vite.js` exists in the cloned repo |
