...
- Automated remediation PRs and remediation-control files are CODEOWNERS-protected for security governance review.
...
**Officer portal queue controls**: When offline, the officer sees an explicit offline-mode message and a live queued-action count. When connectivity returns, the portal auto-attempts sync and shows a success confirmation when the queue is cleared. A manual **Sync now** action is available whenever queued items remain.
...
3. **To take action on a breach:**
   - Click the breach row to open the breach detail panel and Decision Dock.
   - Click **Guided triage flow** to follow the in-page 3-step workflow:
     1. **Choose action** (assign officer, warning, enforcement, notice issue, reject, or resolve)
     2. **Add details** (for example: rejection reason, outcome notes, or assignment handoff)
     3. **Confirm** and execute the action
   - The guided flow reuses existing adjudication handlers and opens notice pages only when required for document issuance.
   - Direct action buttons remain available in the Decision Dock for experienced operators who need rapid handling.
...
The Roster Planner provides a **visual roster board** — officers as rows, days as columns, shift cards in cells — inspired by InTime / Deputy workforce management tools. The board supports both **week** and **fortnight** views.

**Navigating the roster:**

- Use the **← Previous week** / **Next week →** arrows to move between weeks.
- Use the **Week / Fortnight** toggle to switch roster horizon.
...
**Workforce tab (embedded user management):**

- Users with roster admin roles (`admin`, `admin_officer`, `master`) can switch between **Planner** and **Workforce** tabs from the roster header.
- The **Workforce** tab opens an embedded staffing/user-management view for roster operations without leaving `/roster`.
...
- **Start Here hero** — role-aware recommended report (Leadership Pack for admin/master, Patrol Activity for officer) with a guided entry CTA
- **Categorised report list** — grouped report options for compliance, operational, analytics, and system outputs
- **Guided report workflow** — an in-page step flow that keeps the operator in one context while selecting, configuring, previewing, and generating a report

**Generating a report:**
1. Click **Start guided flow** from the recommended card or any report row.
2. **Select report type** — choose the exact report to generate.
3. **Configure filters** — choose output format (PDF/CSV), date preset or custom range, and optional content toggles.
4. **Preview output** — confirm the selected report, date window, and included sections before generation.
5. Click **Generate and download** — the app opens the target report surface with the selected query parameters attached.

**Wizard behaviour:**
- The flow is shown in a dialog and displays explicit progress: **Step 1 of 4** through **Step 4 of 4**.
- Operators can go **Back** without losing entered state.
- The workflow replaces the old "click card -> navigate away" pattern for standard report generation from the hub.
...
**Guided shift workflow (recommended):**
1. From the patrol banner, tap **Guided shift flow**.
2. Follow the 5-step in-page sequence: **Start shift** → **Select zone** → **Open scan** → **Record result** → **Submit report**.
3. Use **Run this step** to execute each action without leaving the officer portal context.
4. Use **Back/Next** to review or advance while keeping your current state.
