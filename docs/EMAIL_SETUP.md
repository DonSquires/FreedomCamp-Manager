...
- General sender mailbox: `donotreply@fcmanager.co.nz`
- Report sender mailbox: `reports@fcmanager.co.nz`
...
| `SMTP_FROM_EMAIL` | **Yes** | — | General sender "From" address (production: `donotreply@fcmanager.co.nz`) |
| `SMTP_FROM_NAME` | No | Varies per function | Display name for the sender (see below) |
| `SMTP_REPORTS_FROM_EMAIL` | No | Falls back to `SMTP_FROM_EMAIL` | Report-email sender (production: `reports@fcmanager.co.nz`) |
...
supabase secrets set \
  ...
  SMTP_PORT=465 \
  SMTP_USERNAME=donotreply@fcmanager.co.nz \
  SMTP_PASSWORD=<hpanel_mailbox_password> \
  SMTP_FROM_EMAIL=donotreply@fcmanager.co.nz \
...
1. Create mailboxes in hPanel: `donotreply@fcmanager.co.nz` and `reports@fcmanager.co.nz`.
