...
- [ ] Continue E1 with the next highest-count target pages: `VehicleManagement` (21 baseline), then `AdminPortal` (20 baseline), then `FieldOfficerPortal` (15 baseline), migrating page query clusters into hooks/services and lowering the published baselines after each verified reduction.
