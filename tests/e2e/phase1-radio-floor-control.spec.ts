...
test.describe('phase1 floor control', () => {
  test('officer floor request and state read stay authorized', async () => {
    ...
    const channelId = `phase1-floor-${Date.now()}`
    const radioToken = await mintRadioToken(jwt, channelId)
    test.skip(!radioToken, 'Could not mint radio token')

    const requestRes = await fetch(`${pttServerUrl}/control/floor-request`, {
      method: 'POST',
      headers: {
    ...
    expect([200, 201, 500, 502, 503]).toContain(requestRes.status)
    expect(requestRes.status).not.toBe(401)
    expect(requestRes.status).not.toBe(403)

    const stateRes = await fetch(`${pttServerUrl}/control/floor-state?channel_id=${encodeURIComponent(channelId)}`, {
      headers: {
        Authorization: `Bearer ${radioToken}`,
      },
    })

    expect([200, 500, 502, 503]).toContain(stateRes.status)
    expect(stateRes.status).not.toBe(401)
    expect(stateRes.status).not.toBe(403)
  })
})
