...
const KNOWLEDGE_PACKS = {
  ...
  railway_services_audit: {
    ...
    key_points: [
      ...
      'CRITICAL: Bob production MUST have CHAT_PROVIDER=ollama and TABULAR_NLP_PROVIDER=ollama. Verified by ops-railway-wiring-audit.yml which runs hourly and hard-errors if these are wrong.',
      'CRITICAL: Bob OLLAMA_BASE_URL must be http://ollama.railway.internal:11434 (port 11434, not 3000). Bob and Ollama must be in the same Railway project for private networking to work.',
      ...
      'CRITICAL: OPENAI_API_KEY must NOT be set in self-contained mode. Bob must not egress to OpenAI. If OPENAI_API_KEY_SET=true on /health, remove it from Railway Variables.',
      ...
      'To assess Railway config: ask user for GET <BOB_URL>/health JSON. Check config.CHAT_PROVIDER, config.TABULAR_NLP_PROVIDER, config.DEPLOY_SIGNATURE, config.SELF_CONTAINED_MODE, config.OPENAI_API_KEY_SET, capabilities.chat_local_ollama_enabled. All must match expected values.',
      ...
      'Wiring audit (ops-railway-wiring-audit.yml) validates: URL consistency, CHAT_PROVIDER=ollama, TABULAR_NLP_PROVIDER=ollama, DEPLOY_SIGNATURE, SELF_CONTAINED_MODE=true, OPENAI_API_KEY_SET=false, inference_api_key_configured=true. Runs every hour at :11 past.',
    ],
  },
  railway_platform: {
    ...
    key_points: [
      'Four Railway services: Bob (inference-service/, port 3000), Proxy (proxy-server/, port 3000), PTT (ptt-server/, port 3002), Ollama (ollama/, port 11434).',
      ...
      'Private network: Bob → Ollama via http://ollama.railway.internal:11434. No public egress. Both must be in same Railway project.',
      'Ollama: version 0.20.2 pinned. Port 11434 (default). OLLAMA_HOST=0.0.0.0:11434. OLLAMA_ORIGINS=*. Bob circuit breaker: 3 failures → 60s cooldown.',
      'Bob Docker: multi-stage (Python ONNX export → Node builder → production image). Non-root nodejs user. 1GB+ RAM for ONNX models.',
      'Deploy workflows: deploy-bob-railway.yml, deploy-ollama-railway.yml, deploy-ptt-railway.yml, deploy-proxy-railway.yml.',
