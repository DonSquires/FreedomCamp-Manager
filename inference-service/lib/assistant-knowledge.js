...
const KNOWLEDGE_PACKS = {
  ...
  tender_writing_knowledge: {
    ...
    key_points: [
      ...
      'Bob multi-model routing for tender generation: (1) Ollama (llama3.1:8b or larger) — primary model, handles full document generation with the system prompt below. If Ollama circuit breaker is open, fall back to (2) OpenAI (gpt-4o-mini via OPENAI_BASE_URL) — better at structured text but costs money. If OpenAI is unavailable, fall back to (3) heuristic template — fill in a structured template with context data from the assessment. Always return which model/provider was used in the response so the user can see it in the UI.',
      'POST /tender/generate endpoint: accepts {generation_type:"application"|"response", context:{extracted_text, issuing_body, key_services, key_requirements, key_dates, reference_number, due_date}, organization_context:{name, psa_licence, nzbn}}. Returns {sections:{cover_letter, executive_summary, services_offered, pricing_notes, team_qualifications, health_and_safety, declaration}, provider:"ollama"|"openai"|"heuristic", model_used}. Bob generates content section by section, grounding each section in the extracted tender context.',
    ],
  },
};
