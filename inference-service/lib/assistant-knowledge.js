...
const KNOWLEDGE_PACKS = {
  ...
  codebase_coding: {
    ...
    key_points: [
      'Tech stack: React 18 + TypeScript + Vite + Tailwind CSS v3 + shadcn/ui. State: Zustand + TanStack Query v5. Forms: react-hook-form + zod. Package manager: npm.',
      ...
      'Build: npm run build (tsc + vite). Dev: npm run dev. Lint: npm run lint. package-lock.json must be committed — CI uses npm ci. Use POST /code/assist for coding guidance.',
    ],
  },
  ui_design_context: {
  ...
  ptt_comms_context: {
    ...
    key_points: [
      ...
      'PTT server (ptt-server/): Node.js + Express + ws. Deployed on VPS infrastructure behind HTTPS/WSS (for example ptt.fcmanager.co.nz) via deploy-voice-server.yml. TURN is self-hosted and should be configured for relay use in restricted networks. Env: PTT_JWT_SECRET (required), PROXY_SECRET (required, shared with Edge Function), PORT, TURN_URL + TURN_USERNAME + TURN_CREDENTIAL. Health: GET /health over HTTPS.',
      ...
      'Privacy: Audio clips stored in ptt-clips Supabase Storage bucket with 24h signed URLs. PTT tokens default to 5-minute expiry. Recordings limited to 60s / 3MB. All voice data is org-scoped and auditable. Privacy Act IPP 5 requires security safeguards on voice data.',
    ],
  },
  supabase_platform: {
  ...
  github_platform: {
    ...
    key_points: [
      ...
      'package-lock.json must be committed — CI runs npm ci. Outdated lockfile = deploy failure.',
      'Bob sync: sync-bob-repo.yml mirrors inference-service/ to DonSquires/Bob repo via BOB_SYNC_PAT token.',
      'Common fix: workflow failed → Actions tab → find run → read error. Lockfile stale → npm install + commit. Secrets expired → regenerate.',
    ],
  },
  vercel_platform: {
    ...
    key_points: [
      'Vercel hosts React/Vite SPA. Build: npm run build. Output: dist/. vercel.json at root.',
      ...
      'Common fix: build fails → npm run build locally. CSP blocks request → add domain to connect-src. Preview auth → add *.vercel.app to Supabase redirect_urls.',
    ],
  },
  ...
  hybrid_stack_platform: {
    ...
    key_points: [
      ...
      'Build commands: npm run dev/build/lint (web), eas build (mobile), supabase functions deploy (Edge Functions), supabase db push (migrations), node server.js (Bob).',
    ],
  },
