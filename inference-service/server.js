...
function generateHeuristicChatReply(message, context = {}, scope = null) {
  ...
  if ((lowered.includes('create') || lowered.includes('add') || lowered.includes('build') || lowered.includes('make')) && lowered.includes('page')) {
    ...
    return `To add a new page in FieldOps: ${steps} Pattern: export default function MyPage() { const { user } = useAuthStore(); ... return <div className="p-6">...</div> }. Import shadcn/ui from @/components/ui/. Add route in App.tsx with RoleRoute. Run npm run build to check. Use POST /code/assist with {question:"create page"} for the full template.`;
  }
  ...
  if (lowered.includes('typescript') && (lowered.includes('config') || lowered.includes('tsconfig') || lowered.includes('strict') || lowered.includes('setting'))) {
    return `FieldOps TypeScript config (tsconfig.app.json): noImplicitAny=false, strictNullChecks=false, skipLibCheck=true. Do NOT tighten these settings — the codebase relies on lenient TypeScript. Path alias @/* → ./src/* is defined in both tsconfig.json and vite.config.ts. Build: npm run build (runs tsc -b && vite build).`;
  }
  ...
  if ((lowered.includes('npm') || lowered.includes('package-lock')) && (lowered.includes('install') || lowered.includes('lock') || lowered.includes('frozen'))) {
    return `package-lock.json must be committed alongside package.json changes. Railway runs npm ci and will fail if package-lock.json is stale or missing. Fix: run npm install, commit the updated package-lock.json. Verify: npm ci should succeed with no changes. Use npm in the root.`;
  }
...
async function generateChatReplyWithOllama(message, history = [], context = {}, systemPromptOverride = null, scope = null) {
  ...
  const enterpriseScannabilityPolicy = [
    'Enterprise response density mode:',
    '- Use short, direct sentences and avoid filler.',
    '- Keep output highly scannable with compact bullets and explicit action labels.',
    '- Ensure each line adds unique actionable value; avoid duplication.',
    '- Prioritize latest user query while using long-term context and short-term history when provided.',
    '- End with exactly two follow-up options labeled OPTION 1 and OPTION 2.',
  ].join('\n');
  try {
...
app.post('/bob/agent-loop', inferenceRateLimit, requireInferenceAuth, async (req, res) => {
  try {
    ...
    const authUserId = req.inferenceAuth?.sub || null;
    const requestedUserId = String(req.body?.user_id || '').trim() || authUserId;
    if (!requestedUserId) {
      return res.status(400).json({ error: 'user_id is required or must be resolvable from auth token' });
    }
    ...
    const result = await executeBobAgentLoop({
      userId: requestedUserId,
      operatorId: String(req.body?.operator_id || req.body?.context?.operator_id || '').trim() || null,
      ...
      generateReply: async ({ message, context, systemPromptOverride: override }) => {
        if (CHAT_PROVIDER === 'ollama') {
          return generateChatReplyWithOllama(message, [], context || {}, override || null, requestScope);
        }

        const text = generateHeuristicChatReply(message, context || {}, requestScope);
        return {
          provider: 'heuristic',
          fallback: false,
          text,
    ...
    return res.json({
      ...
      memory: {
        short_term_count: result.shortTermCount,
        long_term_count: result.longTermCount,
      },
      provider: result.reply.provider,
      fallback: result.reply.fallback === true,
      message: result.reply.text,
      text: result.reply.text,
      quality_gate: result.reply.quality_gate || null,
      quality_gate_failed: result.reply.quality_gate?.status === 'failed',
    });
  } catch (error) {
    console.error('Bob agent loop endpoint error:', error);
    return res.status(500).json({
      error: 'bob agent loop failed',
      message: error.message,
    });
  }
});
