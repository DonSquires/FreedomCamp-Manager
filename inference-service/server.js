...
async function generateChatReplyWithOllama(message, history = [], context = {}, systemPromptOverride = null, runtime = null) {
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    try {
      recordEgressEvent('ollama', 'attempted', `chat response generation attempt ${attempt}`);
      const result = await runChatAttempt();
      ollamaCircuitBreaker.recordSuccess();
      return result;
    } catch (error) {
      lastError = error;
      ollamaCircuitBreaker.recordFailure(error);
      if (ollamaCircuitBreaker.state !== 'open') {
        console.warn(`⚠️ Local chat via Ollama failed (${ollamaBaseUrl}) attempt ${attempt}/2:`, error.message);
      }
      if (attempt < 2) {
        await probeOllamaTags(5000, { workload: 'chat', baseUrl: ollamaBaseUrl });
      }
    }
  }

  if (lastError) {
    recordEgressEvent('ollama', 'failed', `chat fell back after retry: ${lastError.message}`);
  }
  return buildChatHeuristicFallback(message, context);
}
  ...
  try {
    ...
    if (OLLAMA_ENABLED && !SELF_CONTAINED_MODE) {
      ...
      try {
        ...
        planTimeout = setTimeout(() => planController.abort(), CHAT_TIMEOUT_MS);
        const planResp = await safeFetch(
