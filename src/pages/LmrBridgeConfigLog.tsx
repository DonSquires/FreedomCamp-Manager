 ...
 *
 * Admin configuration log for lmr_bridge_config.
 * Shows gateway settings, channel/direction metadata, active state, and notes.
 *
 * Route: /lmr-bridge-config-log — admin/master
 */
...
type ConfigRow = Database['public']['Tables']['lmr_bridge_config']['Row']
...
export default function LmrBridgeConfigLog() {
  ...
  const { data: rows = [], isLoading, refetch } = useQuery<ConfigRow[]>({
    queryKey: ['lmr-bridge-config-log', activeFilter, directionFilter, labelQuery],
    queryFn: async () => {
      let q = supabase
  ...
  const withToken = rows.filter(r => !!r.gateway_token).length
  const directions = ['all', ...Array.from(new Set(rows.map(r => r.direction).filter(Boolean)))]
      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Bridge gateway configuration history and active channel settings</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { label: 'With Token', value: withToken, colour: 'text-indigo-700' },
          ].map(kpi => (
            <Card key={kpi.label}>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Select value={directionFilter} onValueChange={setDirectionFilter}>
            ...
            <SelectContent>
              {directions.map((d) => (
                <SelectItem key={d} value={d}>{d === 'all' ? 'All directions' : d}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center py-12 text-muted-foreground gap-2">
            <AlertCircle className="h-8 w-8" /><p>No bridge configs found</p>
          </div>
        ) : (
                ...
                {rows.map((row) => (
                  <Fragment key={row.id}>
                    ...
                    >
                      ...
                      <TableCell className="text-xs max-w-[12rem] truncate text-muted-foreground" title={row.gateway_url}>
                        {row.gateway_url}
                      </TableCell>
                    ...
                    {expanded === row.id && (
                      <TableRow className="bg-muted/20">
                        <TableCell colSpan={7} className="text-xs text-muted-foreground space-y-1 py-3">
                          ...
                          <div><span className="font-medium">Gateway token:</span> {maskToken(row.gateway_token)}</div>
