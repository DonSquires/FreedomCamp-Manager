...
export default function CanonicalHomelessLog() {
  ...
  const { data: rows = [], isLoading, refetch } = useQuery<CanonicalHomelessRow[]>({
    queryKey: ['canonical-homeless-log', orgId, searchQuery, statusFilter, sourceFilter, dateFrom],
    enabled: !!user,
    queryFn: async () => {
      ...
      if (searchQuery.trim()) {
        ...
        q = q.or(`plate_number.ilike.%${term}%,notes.ilike.%${term}%,confirmed_by.ilike.%${term}%`)
      }
      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Canonical homeless vehicle registry with confirmation status, source, and lifecycle metadata</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { label: 'Confirmed', value: confirmedCount, colour: 'text-amber-700' },
            { label: 'Cleared', value: clearedCount, colour: 'text-green-700' },
            { label: 'Sources', value: sources.length, colour: 'text-slate-700' },
          ].map(kpi => (
            <Card key={kpi.label}>
        ...
        <div className="flex flex-wrap gap-3">
          <Input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search plate, notes, confirmed by…" className="w-64" />
                ...
                {rows.map(row => (
                  <Fragment key={row.plate_number}>
                    <TableRow className="cursor-pointer hover:bg-muted/40" onClick={() => setExpanded(expanded === row.plate_number ? null : row.plate_number)}>
                      ...
                      <TableCell className="text-sm">{row.confirmed_by ?? '—'}</TableCell>
                      <TableCell className="text-sm">{fmtDate(row.confirmed_at)}</TableCell>
