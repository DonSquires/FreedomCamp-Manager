...
export default function ZoneManagement() {
  ...
  const stats = zones ? {
    ...
    dayVisitOnly: zones.filter(z => z.is_day_visit_only).length,
    requiresSC: zones.filter(z => z.requires_self_contained).length,
  } : null
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 space-y-2">
                      ...
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="font-medium">{zone.max_nights_per_month}</span>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-1 mt-2">
                        {zone.is_day_visit_only && (
                          <Badge variant="outline" className="text-xs bg-orange-50">
                            Day Visit Only
                          </Badge>
                        )}
                        {zone.requires_self_contained && (
                    ...
                    <div className="flex items-center justify-between text-sm pt-2 border-t">
                      ...
                      <span className="font-medium">{zone._count?.observations || 0}</span>
                    </div>

                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-600">Breaches:</span>
                      <span className={`font-medium ${(zone._count?.breaches || 0) > 0 ? 'text-red-600' : 'text-green-600'}`}>
                        {zone._count?.breaches || 0}
                      </span>
                    </div>
