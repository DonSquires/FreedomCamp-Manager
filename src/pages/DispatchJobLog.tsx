...
export default function DispatchJobLog() {
  ...
  const exportCsv = () => {
    ...
    const headers = ['Job Number', 'Type', 'Status', 'Priority', 'Title', 'Address', 'Assigned To', 'Created At']
    ...
    anchor.href = url
    anchor.download = `dispatch-job-log-${new Date().toISOString().slice(0, 10)}.csv`
    ...
    <AppLayout>
      <div className="p-6 space-y-6">
        ...
        <div className="grid gap-2 rounded-lg border border-gray-200 p-3 text-xs dark:border-[#9E9E9E]/20 md:grid-cols-4">
          ...
          <div><span className="font-semibold text-amber-700 dark:text-amber-300">Watch:</span> Pending {rows.filter(r => r.status === 'pending').length}</div>
          <div><span className="font-semibold text-orange-700 dark:text-orange-300">Action:</span> Active {activeCount}</div>
          <div><span className="font-semibold text-red-700 dark:text-red-300">Critical:</span> SLA breached {slaBreachedCount}</div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center py-12 text-muted-foreground gap-3">
            ...
            <p>No dispatch jobs match the active filters.</p>
            <Button
              type="button"
              variant="outline"
              size="sm"
