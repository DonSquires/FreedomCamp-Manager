...
async function importBoundaries() {
  ...
  for (const feature of geojson.features) {
    ...
    // Escape SQL pattern characters to avoid unintended wildcard matches
    const escapedName = rawName.replace(/\\/g, '\\\\').replace(/%/g, '\\%').replace(/_/g, '\\_');
    ...
    if (org) {
      ...
      if (orgErr) {
        ...
        // Note: zones use zone_type = 'general' for top-level jurisdiction zones
        const { error: zoneErr } = await supabase
          ...
          .eq('zone_type', 'general'); // Top-level jurisdiction zones
